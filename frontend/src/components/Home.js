...
const destinationsData = [
  ...
  { 
    ...
    date: '04/18/2024',
    activities: ['Opera House Tour', 'Sydney Harbour Bridge Climb', 'Bondi Beach'] 
  },
  { 
    id: 7, 
    country: 'Spain',
  ...
  { 
    ...
    date: '04/18/2024',
    activities: ['Christ the Redeemer Tour', 'Sugarloaf Mountain', 'Copacabana Beach'] 
  },
  { 
    id: 11, 
    country: 'Egypt',
...
function Home() {
  ...
  const activities = selectedDestination ? filterActivitiesByDestination(selectedDestination) : [];
      ...
      <div className="search-bar">
        ...
        <div className="search-box">
          ...
          <select id="country" onChange={handleCountryChange}>
            ...
            {destinationsData.map(destination => (
              <option key={destination.id} value={destination.country}>{destination.country}</option>
            ))}
          </select>
        </div>
        ...
        <div className="search-box travelers-section">
          ...
          <div className="travelers-dropdown">
            <select value={adults} onChange={(e) => handleAdultChange(e.target.value)}>
              {[...Array(10)].map((_, i) => (
                <option key={i} value={i + 1}>{`${i + 1} Adult${i !== 0 ? 's' : ''}`}</option>
              ))}
            </select>
            <select value={children} onChange={(e) => handleChildrenChange(e.target.value)}>
              {[...Array(10)].map((_, i) => (
      ...
      <div className="selected-destinations">
        ...
        {travelPlans.map((plan, index) => (
          <div key={index} className="plan-item">
            ...
            <button onClick={() => handleRemovePlan(index)}>Delete</button>
          </div>
        ))}
      </div>
    </div>
